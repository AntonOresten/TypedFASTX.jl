...
@testset "reader.jl" begin
    ...
    @testset "Show reader" begin
        tr = AbstractReader{LongDNA{4}, NoQuality}(fasta_file)
        io = IOBuffer()
        @test isnothing(show(io, tr))
        @test String(take!(io)) == "DNAReader{NoQuality}(\"data/seqs.fasta\", 1)"

        tr = AbstractReader{LongDNA{2}, NoQuality}(fasta_file)
        io = IOBuffer()
        Base.invokelatest(show, io, MIME("text/plain"), tr)
        str = String(take!(io))
        @test str == "AbstractReader{LongSequence{DNAAlphabet{2}}, NoQuality}\n  path: \"data/seqs.fasta\"\n  position: 1"
    end

    @testset "FASTA with index" begin
        tr = AbstractReader{LongDNA{4}, NoQuality}(fasta_file, true)
        @test tr.path == fasta_file
        @test tr.reader isa FASTA.Reader
        @test tr isa AbstractReader{LongDNA{4}, NoQuality}
    ...
    @testset "FASTA without index" begin
        tr = AbstractReader{LongDNA{4}, NoQuality}(fasta_file, false)
    ...
    @testset "FASTQ" begin
        tr = AbstractReader{LongDNA{4}, QualityScores}(fastq_file)
