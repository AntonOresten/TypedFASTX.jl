@testset "conversion.jl" begin
    ...
    @test_throws ErrorException AARecord(fa_record)
    @test AARecord(fa_record) == AARecord("Mickey", "sMiTh")

    @test DNARecord(fq_record) == DNARecord("Ricky", "TTTAA")
    @test DNARecord(fq_record) == DNARecord("Ricky", "TTTAA", "Smith")
    @test DNARecord(fq_record) == DNARecord("Ricky", "TTTAA", "Smith")
    @test DNARecord(fq_record, :sanger) == DNARecord("Ricky", "TTTAA", "Smith")
